...
from bullcrypt import utils, types, algorithm
...
def _add_parsing_strategy_group(parser: argparse.ArgumentParser) -> None:
    ...
    exclusion_group.add_argument(
        "--raw",
        action="store_true",
        help="Interpret all files as binary ciphertext."
    )
...
def _add_algorithm_group(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "algorithm",
        choices=[e.name for e in algorithms],
    ...
    for alg in algorithms:
        ...
        handler.register_args(alg.name, algorithm_group)
